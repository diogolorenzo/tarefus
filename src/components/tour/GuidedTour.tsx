...
export const GuidedTour: React.FC = () => {
  ...
  const getStepIcon = () => {
    switch (currentStep.id) {
      ...
      case 'step-ai':
        return <AiMark className="w-5 h-5 text-purple-500" />;
      case 'step-filters':
  ...
  return (
    <div className="fixed inset-0 z-[100] pointer-events-auto transition-opacity duration-300">
      ...
      <div
        ...
        className="fixed z-[101] bg-white dark:bg-[#0E1424] border border-slate-200/90 dark:border-indigo-500/30 rounded-3xl p-5 shadow-2xl transition-all duration-300 animate-fade-in flex flex-col gap-4 text-ink"
      ...
      >
        ...
        <div className="flex items-center justify-between pt-1 border-t border-line">
          <button
            type="button"
            onClick={handleSkip}
            className="text-xs font-semibold text-muted hover:text-slate-800 dark:hover:text-slate-200 cursor-pointer"
          >
            Pular tutorial
          </button>
          ...
          <div className="flex items-center gap-2">
            {!isFirstStep && (
              <button
                type="button"
                onClick={handlePrev}
                className="flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-bold text-ink hover:bg-slate-100 dark:hover:bg-white/[0.06] border border-slate-200 dark:border-white/[0.1] transition-all cursor-pointer"
              >
