...
export const BoardModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-fade-in">
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-line bg-slate-50/50 dark:bg-[#161F32]/80">
          <div className="flex items-center gap-2">
            <Building2 className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
            <h3 className="text-base font-bold text-ink">Nova Área / Quadro</h3>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSave} className="p-6 space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 bg-sunken border border-line rounded-xl text-sm font-semibold text-ink focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 dark:focus:border-indigo-400 focus:bg-surface transition-all shadow-2xs placeholder:text-subtle"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 bg-sunken border border-line rounded-xl text-xs sm:text-sm text-ink focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 dark:focus:border-indigo-400 focus:bg-surface transition-all shadow-2xs placeholder:text-subtle"
            />
          </div>

          {/* Color Picker */}
          <div>
            <label className="block text-xs font-bold text-ink uppercase tracking-wider mb-2 flex items-center gap-1">
              <Palette className="w-3.5 h-3.5 text-subtle" /> Cor de Destaque
            </label>
              ...
              {COLOR_OPTIONS.map((c) => (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-medium transition-all cursor-pointer ${
                    color === c.id
                      ? 'border-slate-800 dark:border-indigo-500 ring-2 ring-slate-800/10 dark:ring-indigo-500/30 bg-slate-50 dark:bg-[#192336] text-ink font-bold'
